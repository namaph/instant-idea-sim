...
if "LOCAL" not in os.environ:
    logger.debug(f'LOAD google debugger')
    try:
...
def read_root():
    return T.resp.Home(
        detail="Hello This is Namaph Simulator"
    )
...
def check_result(id: str):
    ...
    return T.resp.Result(
        status = msg[st],
        model_name = str(t),
        hist = pickle.loads(cli.get(id)) if st == 4 else None
    )
...
def get_results():
    ...
    return [
        T.resp._Results(
            id = k,
            timestamp = t.decode('ascii'),
            status = int(s),
            model_name = m.decode('ascii'))
